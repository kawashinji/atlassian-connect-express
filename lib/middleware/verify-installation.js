...
function isJWTAsymmetric(addon, req) {
  ...
  // Asymmetric install hook must have keyId in JWT header to verify
  return !!jwt.getKeyId(token);
}
...
async function verifyAsymmetricJwtAndGetClaims(addon, req) {
  ...
  if (_.isEmpty(unverifiedClaims.aud) || !unverifiedClaims.aud[0] 
    || unverifiedClaims.aud[0].replace(/\/$/, "") != addon.config.localBaseUrl().replace(/\/$/, "")) {
