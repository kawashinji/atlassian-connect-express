...
const ConnectOauth2 = require("./oauth2");
...
class HostClient {
  constructor(addon, context, clientKey) {
    ...
    this.connectOauth2 = new ConnectOauth2(addon);
    this.forgeOauth2 = new ForgeOauth2(addon);
  ...
  modifyArgs(options, augmentHeaders, callback, clientSettings) {
    if (this.usingForgeOAuth2Flag) {
      ...
      const productType = clientSettings.productType;
  ...
  getUserBearerToken(scopes, clientSettings) {
    ...
    if (this.userAccountId) {
      return this.connectOauth2.getUserBearerTokenByUserAccountId(
        this.userAccountId,
        scopes,
        clientSettings
      );
    } else if (this.userKey) {
      return this.connectOauth2.getUserBearerToken(
        this.userKey,
        scopes,
        clientSettings
      );
  ...
  getBearerToken(clientSettings) {
    if (this.usingForgeOAuth2Flag) {
      return this.forgeOauth2.getBearerToken(clientSettings);
    }

    throw new Error("usingForgeOAuth2Flag is OFF. Did you call usingOAuth2()?");
  }
