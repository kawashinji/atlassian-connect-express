...
utils.wrapUrlWithStargate = function (
  stargateUrl,
  requestUrl,
  productType,
  cloudId
) {
  utils.checkNotUndefined(stargateUrl, "stargateUrl");
  ...
  return `${stargateUrl}/ex/${productType}/${cloudId}${requestUrl}`;
};
...
utils.checkOauth2Enabled = function (clientSettings) {
  if (_.isNil(clientSettings.authentication)) {
    throw new Error(
      `Has this connect app really been migrated into Forge? (https://developer.atlassian.com/platform/forge/build-a-connect-on-forge-app)`
    );
  }

  if (clientSettings.authentication !== "oauth2") {
    throw new Error(
      `oauth2 authentication has not been enabled. Please see app.connect.authentication in manifest.yml`
    );
  }
};
